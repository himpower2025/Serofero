...
const ProductDetail = ({ 
  ...
  onToggleCart: () => void;
  onStartChat: () => void;
  onReport: (product: Product, reason: string) => void;
  onBlockSeller: (sellerId: string, sellerName: string) => void;
}) => {
        ...
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-md flex items-center justify-between p-4 border-b border-gray-50">
          ...
          <div className="flex gap-2 relative">
            ...
            <button
              ...
              className="p-2.5 hover:bg-gray-100 rounded-full transition-colors text-gray-500 hover:text-gray-950"
            >
            ...
            {showMoreMenu && (
              <>
                ...
                <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 z-20">
                  <button
                    onClick={() => {
                      setShowMoreMenu(false);
                      onReport(product, 'Reported from listing detail');
                    }}
                    className="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-bold text-gray-700 hover:bg-gray-50 transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    onClick={() => {
                      setShowMoreMenu(false);
                      onBlockSeller(product.seller.id, product.seller.name);
                    }}
...
const SellScreen = ({ 
  ...
  const isFormValid = title && price && category && location && description && photos.length > 0 && !isUploadingPhoto;
...
export default function App() {
  // Real Firebase Authentication state (replaces the old fake `isLoggedIn`
  // boolean that just flipped to true on click with no actual sign-in).
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        // Reflect the real signed-in identity in the profile/listing seller info.
        ME_USER.id = user.uid;
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        // Native (Android/iOS): use the real native Google sign-in sheet
        // instead of a web popup (which often fails to open correctly
        // inside a Capacitor WebView), then mirror that sign-in into the
        // Firebase JS SDK so Firestore/Storage calls see the same user.
      ...
      // onAuthStateChanged above flips currentUser and the app re-renders.
    } catch (err) {
  ...
  const handleStartChatForProduct = async (prod: Product) => {
    if (!currentUser) return;

    // Already-open thread with this seller about this exact item? Just switch to it.
    const existingRoom = chatRooms.find(r => r.product.id === prod.id && r.sellerId === prod.seller.id);
    if (existingRoom) {
    ...
    try {
      const chatId = await getOrCreateChatRoom(prod, currentUser.uid);
      setActiveChatRoomId(chatId);
      setActiveTab('chat');
      setSelectedProduct(null);
      setMobileShowConvo(true);
    } catch (err) {
      console.error('[Firestore] Could not start chat:', err);
      triggerPushNotification(
        "⚠️ Chat unavailable",
        "Could not start this conversation — check your connection and try again."
      );
    }
  };
  ...
  // Apple 1.2 / Play "User Generated Content" safety requirements:
  // users must be able to report objectionable listings and block abusive
  // sellers, and see/manage what they've blocked.
  const [blockedSellers, setBlockedSellers] = useState<{ id: string; name: string }[]>(() => {
    const saved = localStorage.getItem('sero_blocked_sellers');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return [];
  ...
  const handleBlockSeller = (sellerId: string, sellerName: string) => {
    if (blockedSellers.some((s) => s.id === sellerId)) return;
    const updated = [...blockedSellers, { id: sellerId, name: sellerName }];
    setBlockedSellers(updated);
    localStorage.setItem('sero_blocked_sellers', JSON.stringify(updated));
    if (selectedProduct?.seller.id === sellerId) setSelectedProduct(null);

    triggerPushNotification(
      "🚫 Seller Blocked",
      `${sellerName}'s listings and messages are now hidden. Manage blocked sellers anytime from your Profile.`
    );
  };

  const handleUnblockSeller = (sellerId: string) => {
    const updated = blockedSellers.filter((s) => s.id !== sellerId);
    setBlockedSellers(updated);
    localStorage.setItem('sero_blocked_sellers', JSON.stringify(updated));
  };
  ...
  // Live chat rooms from Firestore (replaces the old hardcoded mock threads).
  // Falls back to empty (with a console warning) until Firebase is configured.
  const [chatRooms, setChatRooms] = useState<ChatRoom[]>([]);

  useEffect(() => {
    if (!currentUser) return;
    const unsubscribe = subscribeToMyChats(
      currentUser.uid,
      (rooms) => setChatRooms(rooms),
      (error) => {
        console.warn(
          '[Firestore] Live chats unavailable — chat list will be empty. ' +
          'Configure src/firebase.ts (see FIREBASE_SETUP.md) to go live.',
          error
        );
        setChatRooms([]);
  ...
  // Messages for whichever chat room is currently open. Subscribed
  // separately (rather than embedding all messages in every room) so the
  // room list doesn't have to listen to every conversation's full history.
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToChatMessages(
      activeChatRoomId,
      (messages) => setActiveChatMessages(messages),
      (error) => {
        console.warn('[Firestore] Could not load messages for this chat.', error);
        setActiveChatMessages([]);
      }
    );
    return unsubscribe;
  ...
  const handleCompleteSurvey = (
    ...
    if (surveyProduct && surveySeller) {
      ...
      const targetRoom = chatRooms.find(room => room.product.id === surveyProduct.id && room.sellerId === surveySeller.id);
      const ledgerText = `🤝 Handshake Completed! Mutual feedback registered successfully!\n\n${buyerText}\n\n${sellerText}`;
      if (targetRoom) {
        completeChatWithSystemMessage(targetRoom.id, ledgerText).catch((err) => {
          console.error('[Firestore] Could not record handshake completion:', err);
        });
      }
  ...
  const handleSendMessage = async () => {
    if (!newMessage.trim() || !activeChatRoomId || !currentUser) return;

    const textToSend = newMessage;
    setNewMessage('');

    try {
      await sendChatMessage(activeChatRoomId, currentUser.uid, textToSend);
    } catch (err) {
      console.error('[Firestore] Could not send message:', err);
      triggerPushNotification("⚠️ Message not sent", "Check your connection and try again.");
      setNewMessage(textToSend); // give the text back so nothing is lost
    }
  };
  ...
  if (authChecking) {
    return (
      <div className="fixed inset-0 bg-teal-800 z-[100] flex items-center justify-center">
        <Handshake size={40} className="text-amber-400 animate-pulse" />
      </div>
    );
  }
            ...
            <div className="flex h-[600px] max-w-5xl mx-auto bg-white overflow-hidden rounded-[32px] border border-gray-100 shadow-xl m-4">
              ...
              <div className={cn(
                ...
                <div className="flex-grow overflow-y-auto divide-y divide-gray-50/70 p-2 space-y-1">
                  {chatRooms
                    .filter((room) => !blockedSellers.some((b) => b.id === room.sellerId))
                    .map((room) => {
                        ...
                        if (msg.senderId === 'system') {
                          return (
                            <div key={msg.id} className="flex justify-center my-4">
                              <div className="bg-emerald-50 text-emerald-800 border border-emerald-100 px-4 py-3 rounded-2xl max-w-sm text-center text-[11px] font-bold leading-normal shadow-sm whitespace-pre-line">
                                {msg.text}
                              </div>
                            </div>
                          );
          ...
          {activeTab === 'profile' && (
            <div className="p-4 md:p-6 max-w-2xl mx-auto space-y-6 pb-24">
              ...
              <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-5">
                ...
                {/* Blocked Sellers Management (Apple Guideline 1.2 - UGC safety) */}
                <div className="border-t border-gray-50 pt-4 space-y-2">
                  ...
                  {blockedSellers.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {blockedSellers.map((s) => (
                        <div key={s.id} className="flex items-center justify-between bg-gray-50/70 p-2.5 rounded-xl border border-gray-100/50">
                          <span className="text-[11px] font-bold text-gray-700">{s.name}</span>
                          <button
                            onClick={() => handleUnblockSeller(s.id)}
                            className="text-[9px] font-black uppercase text-teal-800 hover:underline cursor-pointer"
                          >
                            Unblock
                          </button>
                        </div>
              ...
              <div className="pt-4 text-center space-y-4">
                ...
                <p className="text-[10px] text-gray-400 font-medium">
                  Serofero v{__APP_VERSION__} • Developed by Himpower Pvt. Ltd.
                </p>
                <button 
      ...
      <AnimatePresence>
        {isSelling && (
          <SellScreen 
            ...
            sellerId={currentUser?.uid || 'me'}
            onSubmit={async (newProdData) => {
              ...
              try {
                // Persists to Firestore — the live subscription above will
                // pick this up (usually instantly, via Firestore's local
                // write cache) and update the feed for every user.
                await createProduct(newProductData);
              } catch (err) {
                console.warn(
                  '[Firestore] Could not save listing to the live backend — showing it locally only. ' +
                  'Configure src/firebase.ts (see FIREBASE_SETUP.md) to persist real listings.',
                  err
                );
                setProducts(prev => [{ id: `local-${Date.now()}`, ...newProductData }, ...prev]);
              }
